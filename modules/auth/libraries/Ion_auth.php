...
class Ion_auth
{
	...
	public function __construct()
	{
		...
		$query6 = $this->db->query("SELECT users.email,
										users.default_group,
										users.first_name,
										users.last_name,
										users.username,
										...
										users.jabatan,
										users.project,
										concat(coalesce(monthname(`users`.`create_date`),' '),'. ',coalesce(year(`users`.`create_date`),' ')) AS aktif_sejak,
										users_groups.group_id,
										users_groups.access,
										users_groups.access_group,
										users.ha,
										users.foto,
										groups.`name`,
										users.layout_option,
										users.skins
									FROM users LEFT OUTER JOIN users_groups ON users.id = users_groups.user_id
										 LEFT OUTER JOIN groups ON users_groups.group_id = groups.id
		   ...
		   $row6 = $query6->row_array();
			...
			define("id",$row6['id']);
			define("ha",$row6['ha']);
			define("jabatan",$row6['jabatan']);
			define("username",$row6['username']);
			define("aktif_sejak",$row6['aktif_sejak']);
			define("nik",$row6['nik']);
			define("layout_option",$row6['layout_option']);
			define("skins",$row6['skins']);
			define("project",$row6['project']);
			define("access",$row6['access']);
			define("access_group",$row6['access_group']);
			define("email",$row6['email']);
			define("foto",$row6['foto']);
			define("nik",$row6['id']);
			$query2 = $this->db->query("SELECT * FROM site_info Where id=$user->id");
				 if ($query2->num_rows() > 0)
				 {
				 $row2 = $query2->row_array();
					 }

		define("site_name",$row2['site_name']);
		define("tagline",$row2['tagline']);
		define("SITE_ID",$row2['id']);
		define("site_info",$row2['site_info']);
		define("data_element", $homepage->data_element);
		define("logo", $row2['logo']);
		define("favicon", $row2['favicon']);
		define("yandex_verification", $row2['yandex_verification']);
		define("alexaVerifyID", $row2['alexaVerifyID']);
		define("geo_country", $row2['geo_country']);
		define("language",$row2['language']);
		define("fb_admin", $row2['fb_admin']);
		define("baidu_site_verification",$row2['baidu_site_verification']);
		define("google_site_verification",$row2['google_site_verification']);
		define("google_plus", $row2['google_plus']);
		define("project_complete",$row2['project_complete']);
		define("happy_customers",$row2['happy_customers']);
		define("hour_works",$row2['hour_works']);
		define("cups_of_tea",$row2['cups_of_tea']);
		define("linkedin",$row2['linkedin']);
		define("twitter", $row2['twitter']);
		define("facebook",$row2['facebook']);
		define("skype",$row2['skype']);
		define("youtube", $row2['youtube']);
		define("twitter_username",$row2['twitter_username']);
		define("address",$row2['address']);
		define("mail", $row2['mail']);
		define("phone", $row2['phone']);
		define("fax",$row2['fax']);
		define("contact_me", $row2['contact_me']);
		define("my_location", $row2['my_location']);
		define("color", $homepage->color);
